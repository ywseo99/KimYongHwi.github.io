...
이러한 문제점을 해결하기 위해 Facebook에서는 GraphQL이라는 새로운 API 표준을 개발하고 오픈소스화 했다. 가끔 GraphQL을 database관련 기술로 혼동하는 경우가 있는데 GraphQL은 API를 위한 기술이지 database에 종속적이지 않다. SQL이 DB로부터 데이터를 가져오기 위한 언어라면 GraphQL은 API로부터 데이터를 효율적으로 가져오기 위한 언어이다.

GraphQL의 핵심은 client가 필요한 데이터를 정확하게 지정하여 API로부터 원하는 데이터만 전달 받을 수 있다. 또한 고정 데이터 구조를 리턴하는 여러 엔드 포인트 대신 GraphQL 서버는 단일 엔드 포인트 만 노출하고 클라이언트가 요청한 데이터로 정확하게 응답한다. 즉, GraphQL은 client가 데이터를 서버로부터 효율적으로 가져오는 것이 목적이다.
...
- Ask for what you need, get exactly that
  - client에서는 원하는 데이터를 받아올 수 있어 over fetching, under fetching이 없다.
